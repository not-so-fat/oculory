...
// Inline KnowledgeBaseSecurity to avoid import issues with Vercel
class KnowledgeBaseSecurity {
  ...
  setUserPermissions(userId: string, perm: any) {
...
<body>
  <div class="container">
    <h1>Oculory</h1>
    <p class="subtitle">Ask questions about your Yusuke's knowledge</p>
    ...
    <div id="invite-section">
      ...
      <p style="margin-top: 20px;"><a href="#" onclick="showOwnerLogin(); return false;" style="color: var(--card-gray);">Owner login</a></p>
    </div>
    
    <div id="owner-section" style="display: none;">
      <input type="password" id="owner-code" placeholder="Owner code" />
      <button class="btn-primary" onclick="ownerLogin()">Login</button>
      <p class="error" id="owner-error"></p>
      <p style="margin-top: 20px;"><a href="#" onclick="showOwnerLogin(); return false;" style="color: var(--card-gray);">Back to friend login</a></p>
    </div>
    ...
    <div id="chat-section">
      ...
      <div id="messages">
        <div class="message bot">
          <div class="bubble">Hi! Ask me anything about your Yusuke's knowledge base.</div>
        </div>
      </div>
  ...
  <script>
    ...
    async function ownerLogin() {
      ...
      container.innerHTML = '<h1>Owner Dashboard</h1>' +
        '<button class="btn-primary" onclick="createInvite()">Create Invite</button>' +
        '<button class="btn-primary" onclick="listInvites()" style="margin-top:10px">View Invites</button>' +
        '<div id="owner-content" style="margin-top:20px;text-align:left"></div>';
    }
    ...
    async function listInvites() {
      document.getElementById('owner-content').innerHTML = 
        '<div style="background:rgba(146,228,221,0.1);padding:15px;border-radius:8px;margin:10px 0">' +
        '<strong>Demo User</strong><br/>' +
        'Code: HACK2026<br/>' +
        'Status: active<br/>' +
        'Projects: default, personal<br/>' +
        'Layers: all</div>';
    }
...
app.post("/vapi/webhook", async (req, res) => {
  ...
  switch (type) {
    case "conversation-start":
      ...
      res.json({
        response: "Hi! Ask me anything about your Yusuke's knowledge base."
      });
      return;
